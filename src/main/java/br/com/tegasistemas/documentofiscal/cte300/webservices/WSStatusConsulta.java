...
import br.com.tegasistemas.documentofiscal.cte300.webservices.statusservico.CteStatusServicoStub;
...
import br.com.tegasistemas.documentofiscal.cte300.classes.consultastatusservico.CTeConsStatServ;
...
class WSStatusConsulta implements DFLog {
    ...
    private static CTeConsStatServ geraDadosInutilizacao(final CTeConfig config) {
        final CTeConsStatServ consStatServ = new CTeConsStatServ();
        consStatServ.setAmbiente(config.getAmbiente());
        consStatServ.setVersao(CTeConfig.VERSAO);
        consStatServ.setServico(WSStatusConsulta.NOME_SERVICO);
